...
function SpanLink({
  ...
  return (
    <span
      ...
      className={["cursor-pointer select-none truncate max-w-[40vw]", className]
        .filter(Boolean)
        .join(" ")}
    >
      {children}
...
function useBreadcrumbItems(): BreadcrumbItem[] {
  ...
  if (keyChain.length === 0) {
    ...
    if (segments.length >= 2) {
      ...
      const parentItem = MenuConfig[parentKey as keyof typeof MenuConfig] as
        | IMenuItem
        | undefined;

      // Verifica se o parent tem dropdown e se algum filho tem dinamic_id
      if (parentItem && parentItem.dropdown) {
        for (const [childKey, childItem] of Object.entries(
          parentItem.dropdown
        )) {
          ...
          if (
            ...
            const parentPathValue =
              parentPath
                ?.replace(DEFAULT_INTERNAL_PATH, "")
                .replace(/^\/+/, "") || "";
            const childPathValue =
              childMenuItem.path?.replace(/^\/+/, "") || childKey;
            ...
            const dynamicValue =
              params[childMenuItem.dinamic_id] || segments[segments.length - 1];
            items.push({
      ...
      if (parentItem && "dinamic_id" in parentItem && parentItem.dinamic_id) {
        ...
        const dynamicValue =
          params[parentItem.dinamic_id] || segments[segments.length - 1];
        items.push({
          label: customLabel || dynamicValue,
          path: currentPath,
          isLast: true,
      ...
      <div className="flex items-center gap-3 text-2xl! md:text-4xl font-semibold leading-snug max-w-[90vw] overflow-x-auto">
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              {isLast ? (
                <span className="text-beergam-typography-primary text-2xl!">
                  {item.label}
                </span>
              ) : (
                ...
                <>
                  {item.path ? (
                    <SpanLink
                      to={item.path}
                      className="text-beergam-typography-primary/50 text-2xl! opacity-50 hover:opacity-100 transition-colors inline-flex"
                    >
                      {item.label}
                    </SpanLink>
                  ) : (
                    <span className="text-beergam-typography-primary/50 text-2xl! inline-flex">
                      {item.label}
                    </span>
                  )}
                  <span className="text-beergam-typography-primary/50 text-2xl! select-none">
                    ›
                  </span>
                </>
              )}
