...
export default function BeergamButton({
  ...
  return (
    ...
    >
      <>
        <span
          className={`relative text-base! lg:text-lg!  ${fetcher?.completed || fetcher?.error ? "opacity-0" : "opacity-100"} z-10 ${disabled ? "" : animationStyle == "fade" ? "" : "group-hover:text-beergam-white"}`}
        >
          {title}
        </span>
        {fetcher?.error && (
