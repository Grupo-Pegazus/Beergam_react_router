...
export default function SubscriptionPage({
  ...
  const [collapsingPlans, setCollapsingPlans] = useState<Set<string>>(new Set());
  ...
  const getPlanIcon = (plan: Plan) => {
    if (plan.display_name === "Plano Operacional" || plan.display_name === "Influencer iniciante") {
      return <Svg.home />;
    }
    if (plan.display_name === "Plano Tático" || plan.display_name === "Influencer intermediário") {
      return <Svg.building_library />;
    }
    if (plan.display_name === "Plano Estratégico" || plan.display_name === "Influencer avançado") {
      return <Svg.graph />;
    }
  };
  ...
  const arrangePlansWithPopularInMiddle = (plansArray: Plan[], popularIdx: number) => {
    ...
    return [...beforeMiddle, popularPlan, ...afterMiddle];
  };
  ...
  const arrangedPlans = arrangePlansWithPopularInMiddle(sortedPlans, popularIndex);

  // Função para renderizar todos os benefícios de um plano
  const renderBenefits = (plan: Plan) => {
    const benefits = [
      {
        icon: <Svg.check width={20} height={20} tailWindClasses="stroke-beergam-green" />,
        text: (
          <>
            <span className="font-semibold">{plan.benefits.ML_accounts}</span> contas de marketplace
          </>
        ),
      },
      {
        ...
        text: (
          <>
            <span className="font-semibold">{plan.benefits.colab_accounts}</span> colaboradores
          </>
        ),
      },
      {
        ...
        text: (
          <>
            Monitoramento de <span className="font-semibold">{plan.benefits.catalog_monitoring}</span> catálogos
          </>
        ),
      },
      {
        ...
        text: (
          <>
            <span className="font-semibold">{plan.benefits.dias_historico_vendas}</span> dias de histórico de vendas
          </>
        ),
      },
      {
        ...
        text: (
          <>
            <span className="font-semibold">{plan.benefits.marketplaces_integrados}</span> marketplaces integrados
          </>
        ),
      },
      {
        icon: plan.benefits.sincronizacao_estoque ? (
          <Svg.check width={20} height={20} tailWindClasses="stroke-beergam-green" />
        ) : (
      ...
      },
      {
        ...
        text: (
          <>
            Gestão financeira: <span className="font-semibold">{plan.benefits.gestao_financeira}</span>
          </>
        ),
      },
      {
        ...
        text: (
          <>
            <span className="font-semibold">{plan.benefits.dias_registro_atividades}</span> dias de registro de atividades
          </>
        ),
      },
      {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full items-start">
          {isLoading ? (
            ...
            arrangedPlans.map((plan, index) => {
              const isPopular = plan.display_name === sortedPlans[popularIndex]?.display_name;
              return (
                <div
                ...
                >
                  ...
                  <div className="mb-6">
                    ...
                    <div className="mb-4">
                      <div className="inline-flex items-center justify-center w-12 h-12 bg-beergam-blue-primary rounded-xl text-beergam-white overflow-visible">
                        {(plan.display_name === "Plano Operacional" || plan.display_name === "Influencer iniciante") ? (
                          <div style={{ transform: "scale(0.5)", width: "48px", height: "48px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            {getPlanIcon(plan)}
                          </div>
                        ) : (
                          getPlanIcon(plan)
                  ...
                  <div className="flex-1 mb-6 overflow-hidden transition-all duration-300 ease-in-out">
                    {(() => {
                      ...
                      const visibleBenefits = shouldShowAll ? allBenefits : allBenefits.slice(0, 7);
                        ...
                        <div className="space-y-4 transition-all duration-300 ease-in-out">
                          ...
                          >
                            {visibleBenefits.map((benefit, idx) => {
                              ...
                              const shouldAnimateIn = isExpanded && isExtraBenefit && !isCollapsing;
                              const shouldAnimateOut = isCollapsing && isExtraBenefit;
                                ...
                                <div 
                                  ...
                                  style={{
                                    animation: shouldAnimateIn
                                      ...
                                      : undefined,
                                    animationDelay: shouldAnimateIn ? `${(idx - 7) * 40}ms` : undefined,
                                    opacity: shouldAnimateIn ? 0 : shouldAnimateOut ? 1 : 1,
                                  }}
                                >
          ...
          <p className="text-beergam-white text-xs md:text-sm max-w-xl mx-auto">
            Todos os planos incluem suporte 24/7 e garantia de satisfação de 30
            dias
          </p>
        </div>
      </div>
