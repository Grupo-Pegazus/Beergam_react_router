...
import { FormatProfitRange, isMaster } from "~/features/user/utils";
import Svg from "~/src/assets/svgs";
import { Fields } from "~/src/components/utils/_fields";
...
function ElementWrapper({
  ...
  return (
    <div>
      <Fields.label text={label} />
      {!isEditing && !value && <p>Você ainda não preencheu este campo.</p>}
      {access &&
...
function ElementSection({
  ...
  sectionTitle: keyof typeof editingInitialState;
  children: React.ReactNode;
  editingState: typeof editingInitialState;
  setEditingState: React.Dispatch<
    Partial<typeof editingInitialState> | keyof typeof editingInitialState
  >;
}) {
  const editingValue = useMemo(
    () => editingState[sectionTitle],
    [editingState, sectionTitle]
  );
  return (
    <>
      <div className="flex gap-4 items-center">
        <h2 className="text-beergam-blue-primary">{sectionTitle}</h2>
        <button
          ...
          onClick={() => setEditingState(sectionTitle)}
        >
          {editingValue ? (
            <Svg.arrow_uturn_left width={20} height={20} />
          ) : (
        ...
        {React.Children.map(children, (child) => {
          if (React.isValidElement(child)) {
            return React.cloneElement(
              child as React.ReactElement<ElementWrapperProps>,
              {
...
export default function MinhaConta<T extends IUser | IColab>({
  ...
  const [editingState, setEditingState] = useReducer(
    (
      state: typeof editingInitialState,
      action:
        | Partial<typeof editingInitialState>
  ...
  return (
    <div>
      {/* <p>{JSON.stringify(editedUser)}</p> */}
      {isMaster(user) && isMaster(editedUser) && (
        <ElementSection
