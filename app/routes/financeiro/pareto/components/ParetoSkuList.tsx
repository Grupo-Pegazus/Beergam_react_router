...
import type { ParetoTableItem, ParetoTableResponse } from "~/features/financeiro/pareto/typings";
...
interface ParetoSkuListProps {
    data: ParetoTableResponse;
    onChangePage: (page: number) => void;
}

function RankBadge({ rank }: { rank: number }) {
    const colorMap: Record<string, string> = {
        top: "bg-beergam-green-primary/20 text-beergam-green-primary",
        mid: "bg-beergam-orange/20 text-beergam-orange",
        low: "bg-beergam-gray/20 text-beergam-typography-secondary",
    };
    const tier = rank <= 10 ? "top" : rank <= 30 ? "mid" : "low";
    return (
        <span className={`inline-flex items-center justify-center rounded-lg px-2.5 py-1 text-[11px] font-semibold md:text-[12px] ${colorMap[tier]}`}>
            #{rank}
        </span>
    );
}
...
export default function ParetoSkuList({ data, onChangePage }: ParetoSkuListProps) {
        ...
        <div className="grid gap-4 w-full min-w-0 overflow-hidden">
            ...
            {displayItems.length === 0 ? (
                ...
                <div
                    ...
                    style={{ gridTemplateColumns: "minmax(180px, 2fr) repeat(7, minmax(85px, 1fr))" }}
                >
                    {displayItems.map((item: ParetoTableItem) => (
                        <Paper
                            key={item.sku}
                            component="div"
                        ...
                        >
                            <div className="flex min-w-0 items-center gap-3 pr-4 md:pr-6">
                                ...
                                <div className="min-w-[100px] md:min-w-0 flex-1 overflow-hidden">
                                    <TextCensored censorshipKey="pareto_skus">
                            ...
                            <InfoCell
                                ...
                                value={
                                    <TextCensored censorshipKey="pareto_skus">
                                        {(item.units_share * 100).toFixed(2)}%
                                    </TextCensored>
                                }
                            />
                            <InfoCell
                            ...
                            <InfoCell
                                ...
                                value={
                                    <TextCensored censorshipKey="pareto_skus">
                                        {(item.profit_share * 100).toFixed(2)}%
                                    </TextCensored>
                                }
                            />
                            <InfoCell
                                ...
                                value={
                                    <TextCensored censorshipKey="pareto_skus">
                                        {(item.cumulative_share * 100).toFixed(2)}%
                                    </TextCensored>
                                }
                                isLast
                            />
